...
- Categories: `argshuffle`, `pactail` (PAC-guarded tail call — callee name embedded when resolved), `loadglobal`, `loadmov`, `compare`, `pacsign`, `authstub` (named by GOT slot address in dyld extracts), `helper` (small 1–8 instruction ret/direct-branch helper, ≤32 bytes; marked inline so the decompiler folds them away), and `misc` (≥36 bytes or unusual terminal — genuinely complex, left as-is).
